...
def generate_map(env, map_size, handles):
    ...
    # side = 8
    # pos = []
    # for y in range((height - init_num[0]) // 2, (height + init_num[0]) // 2):
    #     pos.append([(width - side) // 2, y, 0])
    ...
    # side = 8
    # pos = []
    # for y in range((height - init_num[1]) // 2, (height + init_num[1]) // 2):
    #     pos.append([(width + side) // 2, y, 0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_round", type=int, default=2000)  # round of total train
    ...
    parser.add_argument('--alg', default='2leveldqn', choices=['dqn', 'drqn', 'a2c', '2leveldqn'])
    # parser.add_argument('--alg', default='dqn', choices=['dqn', 'drqn', 'a2c', '2leveldqn'])
    ...
    if args.alg == '2leveldqn':
        from magent.builtin.tf_model import TwoLevelDeepQNetwork

        RLModel = TwoLevelDeepQNetwork
        base_args = {'batch_size': batch_size,
                     'memory_size': 2 ** 20, 'learning_rate': 1e-4,
                     'target_update': target_update, 'train_freq': train_freq}
