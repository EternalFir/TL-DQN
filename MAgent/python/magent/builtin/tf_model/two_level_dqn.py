...
import numpy as np
import tensorflow.compat.v1 as tf  # this repo must run under tf.v1
...
class TwoLevelDeepQNetwork(TFBaseModel):
    def __init__(self, env, handle, name,
                 ...
                 num_gpu=1, infer_batch_size=8192, network_type=0):  # infer_batch_size:8192 before
        """init a model
        ...
        custom_view_space: tuple
            customized feature space
        """
        TFBaseModel.__init__(self, env, handle, name, "tftwoleveldqn")
        # ======================== set config  ========================
        self.env = env
        self.handle = handle
    ...
    def _create_network(self, input_view, input_feature, use_conv=True, reuse=None):
        ...
        upper1 = tf.layers.conv2d(input_view, filters=kernel_num[2], kernel_size=10, activation=tf.nn.relu,
                                  name="upper1",
                                  reuse=reuse)
        upper2 = tf.layers.conv2d_transpose(upper1, filters=kernel_num[2], kernel_size=4, activation=tf.nn.relu,
                                            name="upper2", reuse=reuse, )
        global_feature = tf.reshape(upper2, [-1, np.prod([v.value for v in upper2.shape[1:]])])
        tf.reduce_mean(global_feature, 0)
        global_out = tf.layers.dense(global_feature, units=hidden_size[0], activation=tf.nn.relu)
        ...
        h_view = tf.add(global_out * 0.01, h_view)  # add global view
